...
<template>
  <div class="h-full flex flex-col bg-zinc-900">
    ...
    <div class="p-3">
      <div class="relative">
        <SearchIcon class="absolute left-3 top-1/2 transform -translate-y-1/2 text-zinc-400" :size="18" />
        <input
          ref="inputRef"
          v-model="inputValue"
          type="text"
      ...
      <div class="mt-3 space-y-2">
        <label class="flex items-center gap-2 cursor-pointer">
          <input
            ...
            @change="(e) => handleFlagChange(MatchFlag.MatchCase, (e.target as HTMLInputElement).checked)"
        ...
        <label class="flex items-center gap-2 cursor-pointer">
          <input
            ...
            @change="(e) => handleFlagChange(MatchFlag.MatchWholeWord, (e.target as HTMLInputElement).checked)"
            class="w-4 h-4 rounded border-zinc-600 bg-zinc-800 text-blue-500 focus:ring-blue-500 focus:ring-offset-0"
          />
      ...
      <ScrollArea class="h-full px-3 pb-3">
        ...
        <div v-else class="space-y-4">
          <div v-for="[page, hits] in Object.entries(grouped)" :key="page">
            <div class="text-xs text-zinc-500 font-semibold mb-2">
              Page {{ Number(page) + 1 }}
            </div>
